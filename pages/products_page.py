...
PRODUCTS_URL = "https://devatlaskm.marketing-logic.ru/objects/products"


class ProductsPage(BasePage):
    """
    Page object for /objects/products.

    The implementation follows the same ideas as MatrixPage:
    - rely on existing test ids where possible;
    - use ARIA roles (table, row, columnheader) instead of brittle selectors;
    - keep structural assumptions soft so tests are stable with dynamic data.
    """
    ...
    def __init__(self, page: Page):
        ...
        self.page_title: Locator = page.get_by_role(
            "heading", name=re.compile("Продукт", re.IGNORECASE)
        )
    ...
    def sort_by_header(self, name_or_index: str | int) -> None:
        """
        Click on a header either by index or by partial name match.
        """
    ...
    def assert_basic_layout_loaded(self) -> None:
        ...
        if "authorization" in self.page.url:
            pytest.skip("User is not authorized for Products page; redirected to authorization page")
